...
function Connection(log, firstStreamId, settings) {
    ...
    this._log = log.child({ component : 'http2-connection' });
...
Connection.prototype._writeControlFrame = function _writeControlFrame(frame) {
    if ((frame.type === 'SETTINGS') || (frame.type === 'PING') ||
        ...
        this._log.debug({ frame : frameTmp }, 'Receiving connection level frame');
...
Connection.prototype._allocatePriority = function _allocatePriority(stream) {
    this._log.trace({ priority : stream._priority }, 'Allocating priority for stream.');
        ...
        for (var priority in this._streamPriorities) {
            ...
            while (bucket.length > 0) {
                for (var index = 0; index < bucket.length; index++) {
                    ...
                    var frameTmp = extend({}, frame);
                    delete frameTmp.data;
                    this._log.trace({ frame : frameTmp }, 'Forwarding outgoing frame');
...
Connection.prototype._receive = function _receive(frame, done) {
    var frameTmp = extend({}, frame);
    delete frameTmp.data;
    this._log.trace({ frame : frameTmp }, 'Forwarding incoming frame');
...
Connection.prototype.close = function close(error) {
    ...
    this._log.error({ error : error }, 'Closing the connection');
    this.push({
        type        : 'GOAWAY',
        flags       : {},
        stream      : 0,
...
Connection.prototype._receiveGoaway = function _receiveGoaway(frame) {
    this._log.error({ error : frame.error }, 'Other end closed the connection');
    this.push(null);
    this._closed = true;
