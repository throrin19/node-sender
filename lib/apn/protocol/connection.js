...
// * **new Connection(logger, firstStreamId, settings)**: create a new Connection
//
...
Connection.prototype._allocateId = function _allocateId(stream, id) {
    ...
    this._log.trace({stream: stream, stream_id : id }, 'Allocating ID for stream.');
...
Connection.prototype._allocatePriority = function _allocatePriority(stream) {
    this._log.trace({stream : stream, priority : stream._priority }, 'Allocating priority for stream.');
        ...
        for (var priority in this._streamPriorities) {
            ...
            while (bucket.length > 0) {
                for (var index = 0; index < bucket.length; index++) {
                    ...
                    this._log.trace({stream : stream, frame : frame }, 'Forwarding outgoing frame');
