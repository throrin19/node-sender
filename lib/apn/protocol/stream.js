...
Stream.prototype._writeUpstream = function _writeUpstream(frame) {
    ...
    this._log.debug({ frame: frameTmp }, 'Receiving frame');
    ...
    if (frame.type === 'HEADERS') {
        ...
        this._log.error("This frame's type is not implemented : ALTSVC");
        // TODO
    } else if (frame.type === 'BLOCKED') {
        this._log.error("This frame's type is not implemented : BLOCKED");
        // TODO
    }
    ...
    // * If it's an invalid stream level frame, emit error
        ...
        frameTmp = extend({}, frame);
        delete frameTmp.data;
        this._log.error({ frame : frameTmp }, 'Invalid stream level frame');
        this.emit('error', 'PROTOCOL_ERROR');
    }
...
Stream.prototype._finishing = function _finishing() {
    ...
    if (lastFrame && ((lastFrame.type === 'DATA') || (lastFrame.type === 'HEADERS'))) {
        var frameTmp = extend({}, lastFrame);
        delete frameTmp.data;
        this._log.debug({ frame: frameTmp }, 'Marking last frame with END_STREAM flag.');
