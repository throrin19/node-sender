...
module.exports = function(dependencies) {
  ...
  function Endpoint(options) {
    ...
    log = options.log.child({ component: 'apn-Endpoint', s: this });
  ...
  Endpoint.prototype._setupHTTP2Pipeline = function _setupHTTP2Pipeline() {
    const serializer = new protocol.Serializer(log);
    const compressor = new protocol.Compressor(log, "REQUEST");
    const deserializer = new protocol.Deserializer(log);
    const decompressor = new protocol.Decompressor(log, "RESPONSE");
  ...
  Endpoint.prototype._connect = function connect() {
    ...
    this._connection = new protocol.Connection(log, 1);
