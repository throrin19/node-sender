 ...
 */
Wns.prototype.send = function (params) {
        ...
        (access_token) => {
            ...
            async.each(params.tokenUrl, (tokenUrl, callback) => {
                ...
                results.forEach(response => {
                    if (response.statusCode != 200 || response.err) {
    ...
    request.catch( // failed
        (err, res) => {
            if (!err) {
                ...
                throw new Error("When obtaining access token : Name : " + body["error"] + ", Desc :  " + body["error_description"]);
            } else {
