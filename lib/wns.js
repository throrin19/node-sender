...
var Wns = function WNS(options) {
    ...
    if (options.hasOwnProperty('log')) {
        ...
        this._log = bunyan.createLogger({ name : 'wns-sender' });
    }
};
...
Wns.prototype._verifyLayout = function (data) {
    ...
    if (data.hasOwnProperty("sound") && !Wns.audioSources[data.sound] && data.type === Wns.types.TOAST) {
        this._log.warn("sound not set or wrong, no sound used");
        delete data.sound;
    }
    return data;
};
...
Wns.prototype.send = function (params, callback) {
    ...
    if (!params.hasOwnProperty("type") || !_.contains(_value(Wns.types), params.type)) {
        data.type = Wns.types.TOAST;
        this._log.warn("type not set or wrong, " + data.type + " used");
    } else {
    ...
    if (!data.hasOwnProperty("template") || !Wns.templateSpecs[data.template] && (data.type != "raw" || data.type != "badge")) {
        ...
        this._log.warn("template not set or wrong, " + data.template + " used");
    }
    if (!params.hasOwnProperty("notificationClass")) {
        this._log.warn("notificationClass, immediate value used");
                ...
                Request.post(options, (err, res) => {
                    if (err) {
                        ...
                        if (jsonResponse.statusCode == 200) {
                            this._log.info({ url : tokenUrl }, "Notification sent");
                            this.emit("transmitted", tokenUrl);
                        } else {
                            let error = {
                                url              : tokenUrl,
                            ...
                            this._log.error(error, "Notification fail");
