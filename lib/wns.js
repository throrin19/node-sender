...
var Wns = function WNS(options) {
    ...
    if (options.hasOwnProperty('log')) {
        logger = options.log.child({ widget_type : 'WNS lib' });
    } else {
        logger = bunyan.createLogger({ name : 'WNS' });
    }
};
 ...
 */
Wns.verifyLayout = function (data) {
    if (data.hasOwnProperty("template")) {
        if (!data.hasOwnProperty("image")) {
            data.image = [];
        }
        if (Wns.templateSpecs[data.template][0] < data.image.length) {
            logger.warn("not all images will be used in the template");
        }
        if (Wns.templateSpecs[data.template][0] > data.image.length) {
            logger.error("Not enough images data too fill the template");
            throw new Error("Not enough images data too fill the template");
        }
        if (Wns.templateSpecs[data.template][1] < data.text.length) {
            logger.warn("not all texts will be used in the template");
        }
        if (Wns.templateSpecs[data.template][1] > data.text.length) {
            logger.error("Not enough texts data too fill the template");
    ...
    if (data.hasOwnProperty("sound") && !Wns.audioSources[data.sound] && data.type === Wns.types.TOAST) {
        logger.warn("sound not set or wrong, no sound used");
        delete data.sound;
    }
    return data;
};
 ...
 */
Wns.obtainAccessToken = function (params) {
...
Wns.prototype.send = function (params, callback) {
    ...
    if (!params.hasOwnProperty("type") || !_.contains([Wns.types.TOAST, Wns.types.TILE, Wns.types.BADGE, Wns.types.RAW], params.type)) {
        ...
        logger.warn("type not set or wrong, " + data.type + " used");
    } else {
    ...
    if (!data.hasOwnProperty("template") || !Wns.templateSpecs[data.template] && (data.type != "raw" || data.type != "badge")) {
        ...
        logger.warn("template not set or wrong, " + data.template + " used");
    }
    if (!params.hasOwnProperty("notificationClass")) {
        logger.warn("notificationClass, immediate value used");
        params.notificationClass = "immediate";
    }
    logger.debug(data);
    var payLoad = data.type === Wns.types.RAW ? params.payload : Wns.generatePayLoad(Wns.verifyLayout(data));
    ...
    Wns.obtainAccessToken({
        ...
        function (access_token) {    // fulfilled
                ...
                request.post(options, function (err, res) {
                    if (err) {
                        ...
                        if (jsonResponse.statusCode == 200) {
                            logger.info("Notification sent to : " + tokenUrl);
                            this.emit("transmitted", "Notification sent to " + tokenUrl);
                        } else {
                            logger.error(tokenUrl + " : " + result.statusCode + " : " + result.errorDescription + " " + result.responseText);
        ...
        function (err, res) {   // failed
            if (res) {
                var jsonResult = res.toJSON();
                var body = JSON.parse(jsonResult.body);
                logger.error("When obtaining access token : Name : " + body["error"] + ", Desc :  " + body["error_description"]);
                callback(new Error("When obtaining access token : Name : " + body["error"] + ", Desc :  " + body["error_description"]));
            } else {
                logger.error("Can't get an access token from microsoft : " + err);
                throw new Error("Can't get an access token from microsoft : " + err);
            }
        }.bind(this)
    );
};
