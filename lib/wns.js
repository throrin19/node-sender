...
var Wns = function WNS(options) {
    ...
    if (options.hasOwnProperty('log')) {
        logger = options.log.child({ component: 'wns-sender', s: this });
    } else {
        logger = bunyan.createLogger({ name : 'wns-sender' });
    }
};
...
Wns.prototype.send = function (params, callback) {
    ...
    this._obtainAccessToken({
        ...
        (access_token) => {    // fulfilled
            options.headers["Authorization"] = "Bearer " + access_token;
            var results = [];
            async.each(params.tokenUrl, (tokenUrl, callback) => {
